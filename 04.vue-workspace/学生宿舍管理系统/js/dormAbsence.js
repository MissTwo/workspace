...
let dormAbsenceRecord = Mock.mock({
    "dormAbsenceRecord|20": [
        {
            "id|+1": 1,
...
// 宿舍管理员的数组对象
let dormAbsenceData = dormAbsenceRecord.dormAbsenceRecord
...
let limit = 5
let currentPage = 1
let pagenation  = new Pagenation(currentPage, limit,dormAbsenceData,$('.mer-pagination'), redrawDormAdminTable)
window.pagenation = pagenation
pagenation.draw()
// // 搜索后绘制表格
$('#search_btn').click(function () {
    searchDormManager(dormAbsenceData, $('input[name="absenceName"]').val(),$('input[name="startTime"]').val(),$('input[name="endTime"]').val());
})

// 搜索宿舍管理员信息方法封装
function searchDormManager(data, name,startTime,endTime) {
    pagenation.data= dormAbsenceData
...
function deleteFromData(id){
    ...
    for (let i = 0; i < dormAbsenceData.length; i++){
        if(dormAbsenceData[i].id===id){
            dormAbsenceData.splice(i, 1)
            break
        }
    }
    pagenation.draw()
