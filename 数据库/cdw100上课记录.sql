...
select DEPTNO, count(1) from emp group by DEPTNO;
#
select *, a.dept_total / b.total from (select DEPTNO, count(1) dept_total from emp group by DEPTNO) a
join (select count(1) total from emp) b;

#创建视图
# create view if not EXISTS emp_total as select count(1) total from emp; 报错
create or replace view emp_total as select count(1) total from emp;
create or replace view dept_total as select DEPTNO, count(1) dept_total from emp group by DEPTNO;

select *, a.dept_total / b.total from dept_total a join emp_total b;

drop view aa;
create or replace view aa as select *, a.dept_total / b.total from (select DEPTNO, count(1) dept_total from emp group by DEPTNO) a
join (select count(1) total from emp) b;

#部门名称做为列名
select 
	((select dept_total from dept_total where deptno = 10) 
	/ (select total from emp_total)) "10",
	((select dept_total from dept_total where deptno = 20) 
	/ (select total from emp_total)) "20",
	((select dept_total from dept_total where deptno = 30) 
	/ (select total from emp_total)) "30";

#定义变量保存公司人数总数
#变量的定义
#set方式
#set @e_count = (select total from emp_total);
#select方式
select @e_count:=total from emp_total, (select @e_count) a;
select @e_count;
select 
	((select dept_total from dept_total where deptno = 10) 
	/ @e_count) "10",
